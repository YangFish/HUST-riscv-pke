...
//find errorline's instruction address
  for( ; i < current->line_ind; ++i, ++cur_line) {
    //sprint("i=%d,addr=%x\n",i,cur_line->addr);
    if(cur_line->addr == epc) 
      break;
  }
  if(i == current->line_ind) 
    panic("can't find errorline!\n");
...
static void handle_instruction_access_fault() { print_error(); panic("Instruction access fault!"); }

static void handle_load_access_fault() { print_error(); panic("Load access fault!"); }

static void handle_store_access_fault() { print_error(); panic("Store/AMO access fault!"); }

static void handle_illegal_instruction() { print_error(); panic("Illegal instruction!"); }

static void handle_misaligned_load() { print_error(); panic("Misaligned Load!"); }

static void handle_misaligned_store() { print_error(); panic("Misaligned AMO!"); }
